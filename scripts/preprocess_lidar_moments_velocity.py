...
tfrecords_path = '/lambda_stor/data/rjackson/lidar_tfrecords/'
...
def create_tf_record(file, time_interval=5, first_shape=None):
    ...
    Zn = grid.mean_velocity.where(grid.snr > 1).fillna(-99).values
    Zn = Zn + 19.35
    Zn = Zn/(2*19.35)
    Zn = np.where(Zn < 0, 0, Zn)
    snr =  grid.snr.where(grid.snr > 1).fillna(-99).values
    snr = snr/30
        ...
        if label > -1:
            example = tf.train.Example(features=tf.train.Features(
                feature={
                    ...
                    'vel_image': _bytes_feature(my_data),
                    'snr_image': _bytes_feature(my_data),
