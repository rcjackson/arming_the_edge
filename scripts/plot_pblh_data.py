...
cbh = ceil_ds['pbl_height_dl'].load()
...
for i in range(10):
    ...
    if i % 5 == 0:
        ax[int(i/5), i % 5].set_yticks([0, 500, 1000, 1500, 2000, 2500, 3000])
        ax[int(i/5), i % 5].set_ylabel('PBL Height [m]')
    else:
    ...
    #ax[int(i/6)].legend()
    ax[int(i/5), i % 5].set_xlim([0, 150])
    ax[int(i/5), i % 5].set_title(str(i + 1))
    pct_lt200 = len(np.argwhere(cbh.values[where] < 150)) / len(where) * 100
    ax[int(i/5), i % 5].text(75, 2500, '%3.1f' % pct_lt200)
...
fig.savefig('pbl_hist.png', bbox_inches='tight')
